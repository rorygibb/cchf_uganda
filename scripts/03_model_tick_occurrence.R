...
# ====================== TICK BRT DISTRIBUTION MODELS =============================


# fits, evaluates and predicts spatially from ensembles of boosted regression tree SDMs for Hyalomma ticks in Africa
...
generateDataset = function(occ, env_layers, num_pseudo=5, k=NA, checkerboard=FALSE, checker_type=2, mask_raster, num_pseudo_points=200){
  ...
  for(i in 1:num_pseudo){
    ...
    # if truncatum or rufipes, downsample away from intensely surveyed areas
    if(occ$Species_current[1] %in% c("Hyalomma truncatum", "Hyalomma rufipes", "Rhipicephalus appendiculatus", "Amblyomma variegatum")){
     rbin = function(x){ rbinom(1, 1, x) }
     occ$exclude = sapply(occ$PointsDensity, rbin)
     pres_data = occ[ occ$exclude == 0 | is.na(occ$exclude), ]
    } else{
      pres_data = occ
    }
    
    # create random pseudoabsences and combine with presences (num points = num occ)
    pa = generatePseudos(pres_data, num_points=nrow(pres_data), mask_raster=mask_raster, weighting="none", ext=extent(mask_raster))
...
# specific predictors for models
predictors = c("bio01_temp_annualmean",
               "bio02_temp_meandiurnalrange",
...
ticks = read.csv("./data/africanticks_cumming1998/Main_tick_database(.txt)/RGLO_AllTicks_May2022.csv", stringsAsFactors=FALSE)
ticks = ticks[ ticks$decade_est >= 1930, ]
ticks$PointsDensity = ticks$PointsDensity_90
ticks$Species_current[ ticks$Species_current == "Boophilus decoloratus" ] = "Rhipicephalus (Boophilus) decoloratus"

# combine with additional occurrences from literature and GBIF
...
ticks_lo = read.csv("./data/africanticks_rg/gbif_lo/final/rhiapp_ambvar_gbif_clean.csv", stringsAsFactors = FALSE) %>%
  dplyr::select(Species, x, y, Country) %>%
  dplyr::rename("Species_current"=Species) %>%
  dplyr::mutate(PointsDensity = 0.001) %>%
  dplyr::select(Species_current, x, y, Country, PointsDensity)

ticks_rg2 = read.delim("./data/africanticks_rg/gbif_rg/0294194-210914110416597/0294194-210914110416597.csv") %>%
  dplyr::select(species, decimalLongitude, decimalLatitude, countryCode) %>%
  dplyr::rename("Species_current"=1, "x"=2, "y"=3, "Country"=4) %>%
  dplyr::mutate(PointsDensity = 0.001, 
                Species_current = "Rhipicephalus (Boophilus) decoloratus") %>%
  dplyr::select(Species_current, x, y, Country, PointsDensity)
dec = data.frame(
  x = nchar(unlist(lapply(strsplit(as.character(ticks_rg2$x), "[.]"), "[", 2))), 
  y = nchar(unlist(lapply(strsplit(as.character(ticks_rg2$y), "[.]"), "[", 2))))
ticks_rg2 = ticks_rg2[ apply(dec, 1, max) != 1 & !is.na(apply(dec, 1, max)), ]  # filter out low precision and missing coords
...
#   theme_minimal() +
#   geom_point(data=ticks[ ticks$Species_current == "Rhipicephalus (Boophilus) decoloratus", ], aes(x, y, col=Species_current), pch=16, size=1) +
#   scale_color_viridis_d(begin=0, end=0.8, option="inferno") +
#   theme(axis.text = element_blank(),
#         axis.title=element_blank())

# species with CCHF associations that are Africa-specific
# if extending, would also include H marginatum but would require European records
species = c("Hyalomma rufipes", "Hyalomma truncatum", "Hyalomma dromedarii", "Hyalomma impeltatum",
            "Amblyomma variegatum", "Rhipicephalus appendiculatus", "Rhipicephalus (Boophilus) decoloratus")

# specify which species to model (index)
nnn = 7
spp = species[nnn]
occ = ticks[ ticks$Species_current == spp, ]

# make spdf 
sp::coordinates(occ) = ~x+y

# thin occurrence points to limit spatial autocorrelation 
occ = thinOccurrences(occ, res=0.1)
...
# checkerboard = 1 for very well sampled species (> 1500 points; i.e. leave-out 50% of points for test set)
# slightly faster learning rate to optimise fitting time
if(spp %in% c("Hyalomma truncatum", "Hyalomma rufipes", "Amblyomma variegatum", "Rhipicephalus appendiculatus", "Rhipicephalus (Boophilus) decoloratus")){
  s = Sys.time()
  brt_ens = fitModelEnsemble(occ, env_layers=envx, species=as.vector(spp), modeltype="BRT", num_pseudo=100, mask_raster=geo_mask, k=NA, checkerboard=TRUE, checker_type = 1, learning_rate=0.0075, num_pseudo_points=1100, filepath="./output/model_outputs/climate_ensemble/fitted_models/")
  hist(brt_ens$auc, main="AUC scores, BRT models")
  e = Sys.time()
  s - e
}

# checkerboard 2 for less well sampled (<500 points; 4fold and spatially structured), slightly slower learning rate
if(spp %in% c("Hyalomma dromedarii", "Hyalomma impeltatum")){
  s = Sys.time()
  brt_ens = fitModelEnsemble(occ, env_layers=envx, species=as.vector(spp), modeltype="BRT", num_pseudo=50, mask_raster=geo_mask, k=NA, checkerboard=TRUE, checker_type = 2, learning_rate=0.001, num_pseudo_points=nrow(occ), filepath="./output/model_outputs/climate_ensemble/fitted_models/")
  hist(brt_ens$auc, main="AUC scores, BRT models")
  e = Sys.time()
  s - e
}
...
getEnsembleParams = function(filepath){
  ...
  files = files[ -grep("ensemblemetadata.csv", files) ]
  ...
  partials = dplyr::left_join(partials, vi_summary[ , c("var", "var2", "MeanRelInfluence")])
...
# predictSpDistribution(filepath = "./output/model_outputs/climate_ensemble/fitted_models/Hyalomma truncatum_BRT_2020-06-24_531097/", envx, aggregation_factor = 2)
# predictSpDistribution(filepath = "./output/model_outputs/climate_ensemble/fitted_models/Hyalomma rufipes_BRT_2020-06-24_743236/", envx, aggregation_factor = 2)
# predictSpDistribution(filepath = "./output/model_outputs/climate_ensemble/fitted_models/Hyalomma dromedarii_BRT_2020-06-24_111728/", envx, aggregation_factor = 2)



# predict uga fine scale for h 
# filepathx = "./output/model_outputs/climate_ensemble/fitted_models/Hyalomma rufipes_BRT_2020-07-05_824893_envonly/"
# envy = crop(envx, extent(shpx[ shpx$NAME_0 == "Uganda", ])+0.5)
...
createMeanPrediction = function(filepath, speciesx, stat="mean"){
  ...
  # get rasters
  ff = list.files(paste(filepath, "geo_preds", sep=""), pattern=".tif", full.names=TRUE)
  ff = ff[ -grep("xml", ff)]
  rr = stack(ff)
...
rrm = createMeanPrediction(filepathx, speciesx=spp, stat="mean")
rrs = createMeanPrediction(filepathx, speciesx=spp, stat="sd")
plot(rrm, col=viridis::magma(200))




# ========================= predict Uganda at fine scale ====================

predictSpDistribution2 = function(filepath, env_rasters, aggregation_factor=NA){
  ...
  # create save dir
  save_dir = paste(filepath, "geo_preds_uga/", sep="")
  dir.create(save_dir)
...
# run fine scale pred for Uganda only
envy = crop(envx, extent(shp0[ shp0$NAME_0 == "Uganda", ])+1)
predictSpDistribution2(filepath = filepathx, envy, aggregation_factor = NA)

# and output mean prediction
createMeanPrediction2 = function(filepath, speciesx, stat="mean"){
  
  # get rasters
  ff = list.files(paste(filepath, "geo_preds_uga", sep=""), pattern=".tif", full.names=TRUE)
  ff = ff[ -grep("xml", ff)]
  rr = stack(ff)
  ...
  writeRaster(rr_mean, file=paste(filepath, "mean_geo/", speciesx, "_", stat, "_uga_pred.tif", sep=""), format="GTiff")
  return(rr_mean)
}

rrm = createMeanPrediction2(filepathx, speciesx=spp, stat="mean")
rrs = createMeanPrediction2(filepathx, speciesx=spp, stat="sd")



# library(raster); library(dplyr); library(ggplot2); library(magrittr)
# rr = raster::raster("./output/model_outputs/climate_ensemble/fitted_models/Rhipicephalus appendiculatus_BRT_2022-05-18_96167/mean_geo/Rhipicephalus appendiculatus_meanpred.tif")
# 
# # mapping theme
...
#   coord_fixed() +
#   maptheme +
#   ggtitle("Amblyomma variegatum")
# ggsave(px, file="./output/ambly_pred.png", device="png", units="in", width=6, height=8, dpi=600)
# 
# 
rr = raster::raster("./output/model_outputs/climate_ensemble/fitted_models/Rhipicephalus appendiculatus_BRT_2022-05-18_96167/mean_geo/Rhipicephalus appendiculatus_mean_uga_pred.tif")
rr = rrm
...
shp = sf::st_read("./data/project_shapefiles/africa_admin0.shp") %>%
  dplyr::filter(NAME_0 == "Uganda")
px = rr %>%
  as.data.frame(xy=TRUE) %>%
  dplyr::rename("pred"=3) %>%
  ggplot() +
  geom_raster(aes(x, y, fill=pred)) +
  geom_sf(data=shp, fill=NA, col="blue", size=0.8) +
  scale_fill_viridis_c(option="magma", direction=-1, na.value="white") +
  maptheme +
  ggtitle("Amblyomma variegatum")
# ggsave(px, file="./output/ambly_pred_uga.png", device="png", units="in", width=5, height=5, dpi=600)
